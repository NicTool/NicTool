...
$filename ||= ask("Location of file you would like to import data from")
    if !$filename;
$nt_user ||= ask("NicTool user you would like to import data as")
    if !$nt_user;
...
$group_id ||= $nt->result->{store}{nt_group_id}
