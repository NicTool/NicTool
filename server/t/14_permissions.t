...
sub test_perms {
    ...
    foreach my $p ( keys %perms ) {
        ...
        noerrok( $group1->refresh ) or die "Couldn't refresh test group $gid1";
...
sub test_modify {
    ...
    noerrok($group);
    foreach ( keys %group ) {
        is( $group->get($_), $group{$_}, 'settings should not have changed' );
    }
    ...
    noerrok($group);
    foreach ( keys %group ) {
        is( $group->get($_), $group{$_}, 'changed group settings' );
    }
