...
while (<CONFIG>) {
    ...
    if ( $line = m/^([\d+\.?]+)\/(\d+)\s+(\S+)\s?(\S+)?/ ) {
        ...
        if ( $3 ~~ @configAllowedTokens ) {
            ...
            print "[CONFIG] Parse error: [" . $line . "] (Configuration [$3] not allowed!)\n";
            die;
        }
    }
...
sub ripe_create {
    ...
    my $decoded;
    eval { $decoded = decode_json($json); } or do {
...
sub check_arpa_zone_backup {
    ...
    if ( scalar @z eq 4 ) {    #/16
        ...
        print
            "Zone $zone has id $zid  in group $grp and the /16 $arpa16 in group $grp16 has id $zid16 \n";
    }
}
...
sub get_zone_records_advanced {
    ...
    if ($debug) {
        ...
        for ( my $i = 0; $i < scalar( @{ $r->{store}{records} } ); $i++ ) {
            ...
            print(    $r->{store}{records}[$i]{store}{name} . ","
...
sub get_zone_id {
    ...
    foreach my $nt_group_id (@nt_groups) {
        ...
        if ( $r->{store}{zones}[0]{store}{nt_zone_id} ) {
            print "Found zone $zone in group $nt_group_id with id: "
                . $r->{store}{zones}[0]{store}{nt_zone_id} . "\n";
