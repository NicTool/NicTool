...
foreach my $zone (@zones) {
    ...
    $dns = Net::DNS::Resolver->new( nameservers => \@ns, );
        ...
        if ( &checkResponse($resp) ) {
            ...
            if ( $resp->{total} == 1 ) {
                ...
                if ( &checkResponse($resp) ) {
                    ...
                    warn(     "\t*** Failed to delete zone $zone: "
                            . $resp->{error_desc} . " : "
                            . $resp->{error_msg}
                            . "\n" );
            ...
            warn(     "\t*** Failed searching for zone $zone: "
                    . $resp->{error_desc} . " : "
                    . $resp->{error_msg}
                    . "\n" );
        }
...
sub checkResponse {
    ...
    if ( $resp->{error_msg} ne "OK" ) {
        warn(
            $resp->{error_code} . " - " . $resp->{error_desc} . ": " . $resp->{error_msg} . "\n" );
        return 0;
    }
