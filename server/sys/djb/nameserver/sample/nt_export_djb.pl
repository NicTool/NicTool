#!/usr/bin/perl
#
# $Id: nt_export_djb.pl 665 2008-10-01 01:14:04Z matt $
#
...
my $closes=0;
my @oldargv=@ARGV;
my ($nsid, $rsync, $make, $opt_dbids, $opt_noserials,$do_build_cdb,$ck_md5,$force);
my %options = ( 'make=s'  => \$make,
              'nsid=i'    => \$nsid,
              'r'         => \$rsync,
              'dbids'     => \$opt_dbids,
...
unless ($nsid) {
	die "usage: $0 -nsid X [-make command|-buildcdb] [-md5] [-r] [-dbids] [-noserials] [-force]\n
  Required parameters:
...
chdir($cwd);
warn "$ns->{name}: build_data: $sec_build_data build_cdb: $sec_build_cdb rsync_cdb: $sec_rsync_cdb\n" if $DEBUG;
...
while ($runningtime < $ns->{export_interval}) {
	...
	&set_export_status($ns, $dbh_write, "next update starts in $waitleft seconds") if ($prev_export_interval != $ns->{export_interval});
    $dbh_write->disconnect;
    $closes++;
    $dbh_read->disconnect;
    $closes++;
...
sub get_ns {
    ...
    my $sql = "SELECT * FROM nt_nameserver WHERE nt_nameserver_id = "
			. $dbh_read->quote($nsid);
    my $sth = $dbh_read->prepare($sql); warn "$sql\n" if $DEBUG_SQL;
...
sub db_object_read {
    my $self = shift;
	my @env_vars = qw(NT_DB_TYPE NT_DB_NAME NT_DB_HOST_NAME NT_DB_USER_NAME NT_DB_PASSWORD);
    ...
    my @connect = ("dbi:$engine:database=$database:host=$host", $user, $password);
...
sub rsync_cdb {
    ...
    foreach my $rhost ( @nslist ) {
      ...
      for( my $limit = 0; $limit < 3; $limit++) {
              ...
              $ret = system("$rsync_path -az -e ssh data.cdb tinydns\@$rhost:$ns->{'datadir'}/data.cdb");
              warn "$rsync_path -az -e ssh data.cdb tinydns\@$rhost:$ns->{'datadir'}/data.cdb\n" if $DEBUG;
              warn "($ns->{'name'}) rsync ret = $ret! ($limit)\n" unless ($ret == 0);
              $rsync_fail=1 if $ret!=0;
              $limit = 3 if ($ret == 0 || $ret == 5120); # 5120 = user interrupt (ctrl-c)
      }
...
sub start_export_log {
    my ($ns, $date_start) = @_;
    my $sql = "INSERT INTO nt_nameserver_export_log(nt_nameserver_id, date_start) VALUES ($ns->{nt_nameserver_id}, $date_start)";
    $dbh_write->do($sql); warn "$sql\n" if $DEBUG_SQL;
...
sub finish_export_log {
    ...
    my $sql = "UPDATE nt_nameserver_export_log set " . join(',', @stats) . ", date_finish = $date_finish WHERE nt_nameserver_export_log_id = $row";
    $dbh_write->do($sql); warn "$sql\n" if $DEBUG_SQL;
}

sub get_export_status{
	my ($ns, $dbh) = @_;
	my $sql = "SELECT status FROM nt_nameserver_export_procstatus WHERE nt_nameserver_id = " . $dbh->quote($ns->{nt_nameserver_id});
...
sub set_export_status {
	...
	unless ($ns_status_exist) {
		$sql = "SELECT nt_nameserver_id FROM nt_nameserver_export_procstatus WHERE nt_nameserver_id = "
			 . $dbh->quote($ns->{nt_nameserver_id});
		my $sth = $dbh->prepare($sql); warn "$sql\n" if $DEBUG_SQL;
		$sth->execute;
		if (my $nshash = $sth->fetchrow_hashref) {
			$sql = "UPDATE nt_nameserver_export_procstatus set timestamp = " . time() . ", status = " . $dbh->quote($status) . " WHERE nt_nameserver_id = $ns->{nt_nameserver_id}";
		} else {
			$sql = "INSERT INTO nt_nameserver_export_procstatus(nt_nameserver_id, timestamp, status) VALUES ($ns->{nt_nameserver_id}," . time() . "," . $dbh->quote($status) . ")";
		}
		$ns_status_exist = 1;
    } else {
		$sql = "UPDATE nt_nameserver_export_procstatus set timestamp = " . time() . ", status = " . $dbh->quote($status) . " WHERE nt_nameserver_id = $ns->{nt_nameserver_id}";
