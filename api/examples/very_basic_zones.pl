...
# Zones.pl by Matt Simerson - <msimerson@interland.com> - v 0.82
#   Thanks to Jason Noble for Perl Wizardy & Dajoba for NicTool
#
# ChangeLog
...
my($mailip, $delete, $do_reverse, $debug, @zones, $file);

##
# If we're being called as a CGI, grab all the env vars
# otherwise, get the options from the command line
##
	my %options = (
   	'user=s' 	=> \$user,
   	'pass=s'		=> \$pass,
...
if ( $file ) { if ( -r $file ) {
	open(INPUT, $file ) or die "Unable to open input $file: $!\n";
		while ( <INPUT> ) { chomp $_; my @r = split(/ /, $_); push(@zones, lc($r[0])); };
	close (INPUT);
}; };
...
my $me = &nictool_connect();  # session string is stored for further queries.
	...
	&delete_zone_records($record, $zoneid, $z);
	#&delete_zone($zoneid, $z);
...
sub update_zone_records {		
	...
	if ( $_[3] ne "" ) {
		my $new = &zone_record_template($_[1], $_[2], $_[3], $_[4], $newip, $mailip, $debug);
		...
		for ( my $i = 0; $i < scalar(@{$new}); $i++ ) {
			for ( my $old = 0; $old < scalar(@{$_[0]}); $old++ ) {
				if ( $_[0]->[$old]->{'type'} eq $new->[$i]->{'type'} && $_[0]->[$old]->{'name'} eq $new->[$i]->{'name'} ) {
					print "updating: $_[0]->[$old]->{'nt_zone_id'}, $_[0]->[$old]->{'name'}, $_[0]->[$old]->{'type'} from $_[0]->[$old]->{'address'} to $new->[$i]->{'address'}\n" if ($debug);
					%zone_record = ( 
						nt_zone_record_id => $_[0]->[$old]->{'nt_zone_record_id'},
						...
						weight		=>	$new->[$i]->{'weight'},
						description => $_[0]->[$old]->{'description'}
					);
					...
					print "No match: $new->[$i]->{'type'}, $new->[$i]->{'name'} !=  $_[0]->[$old]->{'type'}, $_[0]->[$old]->{'name'}\n" if ($debug);
					$updated="n";
				};
			};
			if ( $updated ne "y" ) {
		...
		for ( my $i = 0; $i < scalar(@{$_[0]}); $i++ ) {
			...
			} elsif ( $_[0]->[$i]->{'type'} eq "MX" ) {
				if ( $_[0]->[$i]->{'name'} eq "$_[2]\." && $_[0]->[$i]->{'address'} eq "mail.$_[2]\." ) {
					print "mx for $_[2] is ok\n";
				} elsif ( $_[0]->[$i]->{'name'} eq "$_[2]\." && $_[0]->[$i]->{'address'} eq $oldip ) {
					print "WARNING: mx record must be a FQDN (see RFC 1035) - updated from $_[0]->[$i]->{'address'} to mail\.$_[2]\.\n";
					&edit_zone_record( 
						$_[0]->[$i]->{'nt_zone_record_id'}, 
						$_[1], 
						$_[0]->[$i]->{'name'}, 
					...
					print "WARNING: mx for $_[2] is unverified\n";
				};
			};
		};
	};
