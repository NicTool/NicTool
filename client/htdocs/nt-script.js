...
function changeNewZoneName () {
  const zoneName = $('input#zone')
  const mailAddr = $('input#mailaddr')
...
function changeNSExportType (eType) {
  if (!eType) {
    eType = $('select#export_format option:selected').val()
  }
...
function setFormRRTypeSSHFP () {
  ...
  const algoTypes = { '1': 'RSA', '2': 'DSA', '3': 'ECDSA', '4': 'Ed25519', '6': 'Ed448' }
...
function setFormRRTypeDNSKEY () {
  ...
  const algoTypes = getDnssecAlgorithms()
...
function addValuesToSelect (array,selectName) {
  const selObj = $('select#'+selectName).show()
