...
sub do_delete_delegation {
    ...
    if ( $q->param('type') ne 'record' && $q->param('nt_zone_id') ) {
        ...
        if ( $error->{'error_code'} != 200 ) {
            ...
            $nt_obj->display_nice_message( "The zone delegation was successfully removed.",
                "Delegation Removed" );
        }
    }
        ...
        if ( $error->{'error_code'} != 200 ) {
            $nt_obj->display_nice_error( $error, "Delete Zone Record Delegation" );
        }
        else {
 ...
 <tr class=light_grey_bg>
  ...
  <td class="nowrap width1">];
        ...
        if ( !$zone->{'deleted'} && $user->{zone_delegate} ) {
            print
                qq[<a href="javascript:void window.open('delegate_zones.cgi?obj_list=$zone->{'nt_zone_id'}&amp;nt_group_id=$del->{'nt_group_id'}&amp;edit=1', 'delegate_win', 'width=640,height=480,scrollbars,resizable=yes')">Edit</a>];
        }
        else {
  ...
  <td class="nowrap center width1" title="Remove Delegation">];
        ...
        if ( !$zone->{'deleted'} && $user->{zone_delegate} ) {
            ...
            print
                qq[<img src="$NicToolClient::image_dir/trash-delegate-disabled.gif" alt="disabled">];
        }
...
# Display the RR header: Resource Records  New Resource Record | View Resource Record Log
 ...
 <tr class=$bgclass>];
  ...
  <td>];
                ...
                my $edit_url =
                    "zone.cgi?$state_string&amp;nt_zone_record_id=$r_record->{'nt_zone_record_id'}&amp;nt_zone_id=$zone->{'nt_zone_id'}&amp;nt_group_id=$gid&amp;edit_record=1#RECORD";
                ...
                if ( $r_record->{'delegated_by_id'} ) {
                    ...
                    print
                        qq[&nbsp;&nbsp;<img src="$NicToolClient::image_dir/perm-$write.gif" alt="delegate write permission">];
                }
                print qq[
  </td>];
            }
  ...
  <td class=center>];
        ...
        {
            print
                qq[<a href="javascript:void window.open('delegate_zones.cgi?type=record&amp;obj_list=$r_record->{'nt_zone_record_id'}&amp;nt_zone_id=$r_record->{'nt_zone_id'}', 'delegate_win', 'width=640,height=480,scrollbars,resizable=yes')">
    <img src="$NicToolClient::image_dir/delegate.gif" alt="Delegate Resource Record"></a></td>];
        }
        else {
            print
                qq[<img src="$NicToolClient::image_dir/delegate-disabled.gif" alt="delegate disabled"></td>];
        }
...
sub display_zone_records_new {
    ...
    $nt_obj->display_nice_message( "New Zone Record '$data{name}' Created", "New Zone Record" );
}
...
sub display_zone_records_edit {
    ...
    $nt_obj->display_nice_message( "Zone Record successfully modified.", "Edit Zone Record" );
}
...
sub display_zone_records_delete {
    ...
    if ( $error->{'error_code'} != 200 ) {
        ...
        $nt_obj->display_nice_message( "Zone Record successfully deleted.", "Delete Zone Record" );
    }
}
...
sub display_zone_records_head {
    ...
    my $options =
        qq[<li class=first><a href="zone_record_log.cgi?nt_group_id=$gid&amp;nt_zone_id=$zid">View Resource Record Log</a></li>
   ];
    ...
    if ( !$zone->{'deleted'} && $user->{'zonerecord_create'} && $has_dperm ) {
        $options .=
            qq[<li><a href="zone.cgi?nt_group_id=$gid&amp;nt_zone_id=$zid&amp;$state_string&amp;new_record=1#RECORD" onClick="selectedRRType();">New Resource Record</a></li>];
    }
    else {
...
sub display_zone_records_columns {
    ...
    if ( grep { $_->{type} =~ /^(?:MX|SRV|URI|CAA|DS|IPSECKEY|DNSKEY|SSHFP|NAPTR)$/ }
        @$zone_records )
    {
        push @columns, 'weight';
    }
...
sub display_edit_record {
    ...
    my ( $zone_record, $action, $message2 ) = _display_edit_record_action( $nt_obj, $q, $zone );
    ...
    my $modifyperm = !$isdelegate && $user->{'zonerecord_write'}
        || $isdelegate && $user->{'zonerecord_write'} && $zone_record->{'delegate_write'};

    my $default_record_type = $zone_record->{type} || $q->param('type');
    $default_record_type = 'PTR' if $zone->{zone} =~ /(in-addr|ip6)\.arpa/;

    my $rr_type_popup =
        _build_rr_type( $nt_obj, $q, $zone, $zone_record, $default_record_type, $modifyperm );
...
<table class="fat">
 ...
 <tr id=address_row class="light_grey_bg">
  ...
  <td id=address_data class="fat">],
        _build_rr_address( $q, $zone_record, $modifyperm ),
        $nt_obj->help_link('rraddress'),
        qq[
  </td>
 </tr>
 <tr id=weight_row class="light_grey_bg">
  <td id=weight_label class="right"> Weight:</td>
  <td id=weight_data class="fat">], _build_rr_weight( $q, $zone_record, $modifyperm ), qq[
  </td>
 </tr>
 <tr id="priority_row" class="light_grey_bg">
  <td id=priority_label class="right"> Priority:</td>
  <td id=priority_data class="fat">], _build_rr_priority( $q, $zone_record, $modifyperm ), qq[
  </td>
 </tr>
 <tr id="other_row" class="light_grey_bg">
  <td id=other_label class="right">Other:</td>
  <td id=other_data class="fat">], _build_rr_other( $q, $zone_record, $modifyperm ), qq[
  </td>
 </tr>
 ...
 <tr id=timestamp_row class="light_grey_bg">
  ...
  <td id=timestamp_data class="fat">],
        _build_rr_timestamp( $q, $zone_record, $modifyperm ),
        $nt_obj->help_link('timestamp'), qq[
  </td>
 </tr>
 <tr id=location_row class="light_grey_bg">
  <td id=location_label class="right"> Location:</td>
  <td id=location_data class="fat">],
        _build_rr_location( $q, $zone_record, $modifyperm ),
    ...
    <tr class=light_grey_bg>
     ...
     <td class="nowrap width1">];
        ...
        if ( $user->{zonerecord_delegate} ) {
            print
                qq[<a href="javascript:void window.open('delegate_zones.cgi?type=record&amp;obj_list=$zone_record->{'nt_zone_record_id'}&amp;nt_zone_id=$zone_record->{'nt_zone_id'}&amp;nt_group_id=$del->{'nt_group_id'}&amp;edit=1', 'delegate_win', 'width=640,height=480,scrollbars,resizable=yes')">Edit</a>];
        }
        else {
     ...
     <td class="nowrap center width1">];
        ...
        if ( $user->{zonerecord_delegate} ) {
            ...
            print
                qq[<img src="$NicToolClient::image_dir/trash-delegate-disabled.gif" alt="trash delegate disabled">];
        }
...
<table class="fat">
 <tr class=light_grey_bg>
  <td class=left> ];
    ...
    {
        print
            qq[<a href="javascript:void window.open('delegate_zones.cgi?type=record&amp;obj_list=$zone_record->{'nt_zone_record_id'}&amp;nt_zone_id=$zone_record->{'nt_zone_id'}', 'delegate_win', 'width=640,height=480,scrollbars,resizable=yes')">Re-Delegate</a>];
    }
    else {
...
<table class="fat">
 ...
 <tr class=light_grey_bg>
  ...
  <td class="width80">\n];
    ...
    my $ns_tree =
        $nt_obj->get_usable_nameservers( nt_group_id => scalar( $q->param('nt_group_id') ), );
 ...
 <tr class=light_grey_bg>
  ...
  <td class="width80">],
        ...
        -size      => 25,
        -maxlength => 512,
        -default   => $zone->{'mailaddr'} eq 'hostmaster.ZONE.TLD.'
        ? qq[hostmaster.$zone->{zone}.]
        : $zone->{mailaddr},
        ),
